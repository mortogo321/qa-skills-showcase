...
test.describe
	.serial('Restful-Booker CRUD', () => {
		...
		test('POST /booking creates a booking and echoes the submitted fields', async ({
			request,
		}) => {
		...
		test('POST /auth returns a token for the update step', async ({
			request,
		}) => {
		...
		test('PUT /booking/{id} performs a full update using the auth token', async ({
			request,
		}) => {
		...
		test('DELETE /booking/{id} removes the booking, then GET returns 404', async ({
			request,
		}) => {
